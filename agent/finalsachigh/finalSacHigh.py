...
from agent.finalsachigh import models
from agent.finalsachigh import replaybuffer
...
ROOT_PATH = 'finalsachigh'
REPLAYBUFFER_SIZE = 30

class FinalSACHigh(BaseAgentNP):
  MODEL_FILES = ['policy.modelb', 'q1.modelb', 'q2.modelb']
  logger = EpochLogger(output_dir='finalsachigh/logs', output_fname='progress.csv')
  ...
  def initialize(self, batch_size, initial_capital, n_players):
    ...
    self.ranktable = np.stack((
      np.load(os.path.join("agent", "finalsachigh", "5card_rank_percentile.npy")),
      np.load(os.path.join("agent", "finalsachigh", "6card_rank_percentile.npy")),
      np.load(os.path.join("agent", "finalsachigh", "7card_rank_percentile.npy"))
    ))

    self.preflop_table = np.load(os.path.join("agent", "finalsachigh", "preflop_ranks.npy")).tolist()
