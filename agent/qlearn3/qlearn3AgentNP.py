...
from agent.qlearn3 import models
from agent.qlearn3 import replaybuffer
...
class Qlearn3AgentNP(BaseAgentLoadable):
  @classmethod
  def _config_file_path(cls):
    return './agent/qlearn3/config.json'
  ...
  def initialize(self, batch_size, initial_capital, n_players):
    ...
    self.possible_actions = torch.zeros((self.BATCH_SIZE, self.act_dim), device=self.config['setup']['device'])
