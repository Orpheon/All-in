...
from agent.qlearn6 import models
from agent.qlearn6 import replaybuffer
...
class Qlearn6AgentNP(BaseAgentNP):
  MODEL_FILES = ['q.modelb']
  logger = EpochLogger(output_dir='qlearn6/logs', output_fname='progress.csv')
  ...
  def initialize(self, batch_size, initial_capital, n_players):
    ...
    self.ranktable = np.stack((
      np.load(os.path.join("agent", "qlearn6", "5card_rank_percentile.npy")),
      np.load(os.path.join("agent", "qlearn6", "6card_rank_percentile.npy")),
      np.load(os.path.join("agent", "qlearn6", "7card_rank_percentile.npy"))
    ))

    self.preflop_table = np.load(os.path.join("agent", "qlearn6", "preflop_ranks.npy")).tolist()
