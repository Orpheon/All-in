...
from agent.finalsaclow import models
from agent.finalsaclow import replaybuffer
...
ROOT_PATH = 'finalsaclow'
REPLAYBUFFER_SIZE = 30

class FinalSACLow(BaseAgentNP):
  MODEL_FILES = ['policy.modelb', 'q1.modelb', 'q2.modelb']
  logger = EpochLogger(output_dir='finalsaclow/logs', output_fname='progress.csv')
  ...
  def initialize(self, batch_size, initial_capital, n_players):
    ...
    self.ranktable = np.stack((
      np.load(os.path.join("agent", "finalsaclow", "5card_rank_percentile.npy")),
      np.load(os.path.join("agent", "finalsaclow", "6card_rank_percentile.npy")),
      np.load(os.path.join("agent", "finalsaclow", "7card_rank_percentile.npy"))
    ))

    self.preflop_table = np.load(os.path.join("agent", "finalsaclow", "preflop_ranks.npy")).tolist()
