...
from agent.qlearn5 import models
from agent.qlearn5 import replaybuffer
...
class Qlearn5AgentNP(BaseAgentNP):
  MODEL_FILES = ['q.modelb']
  logger = EpochLogger(output_dir='qlearn5/logs', output_fname='progress.csv')
  ...
  def initialize(self, batch_size, initial_capital, n_players):
    ...
    self.possible_raises = np.array([0, 0, 4, 8, 16, 32, 64, 100, 200])

    self.ranktable = np.stack((
      np.load(os.path.join("agent", "qlearn5", "5card_rank_percentile.npy")),
      np.load(os.path.join("agent", "qlearn5", "6card_rank_percentile.npy")),
      np.load(os.path.join("agent", "qlearn5", "7card_rank_percentile.npy"))
    ))

    self.preflop_table = np.load(os.path.join("agent", "qlearn5", "preflop_ranks.npy")).tolist()
